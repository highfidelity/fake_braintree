  ...
  class SinatraApp < Sinatra::Base
    ...
    post '/merchants/:merchant_id/transactions' do
      if FakeBraintree.decline_all_cards?
        ...
        id = md5("#{params[:merchant_id]}#{Time.now.to_f}")
        transaction_from_request = hash_from_request_body_with_key('transaction')
        amount = transaction_from_request['amount']
        options = transaction_from_request['options']

        transaction = FakeBraintree::Transaction.new(id, amount, options)
